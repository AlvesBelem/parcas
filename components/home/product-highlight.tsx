  ...
  return (
    <article className="flex h-full w-full min-h-[430px] flex-col rounded-3xl border border-[#eaded5] bg-white p-4 text-[#3f2b22] shadow-[0_12px_35px_rgba(63,33,25,0.08)]">
      <div className="flex-1 space-y-3">
        <div className="relative h-44 overflow-hidden rounded-2xl border border-[#f3e7de] bg-[#fff8f3]">
          <Image
            src={coverImage}
            alt={product.name}
            fill
            ...
            {gallery.map((image, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border ${active ? "border-[#b02b24] ring-1 ring-[#f3cfc6]" : "border-[#eaded5]"} bg-white transition`}
      ...
      <div className="mt-auto pt-3">
        <Button
          ...
          variant="primary"
        >
          <Link href={`/out/product/${product.slug}`} prefetch={false} className="inline-flex w-full items-center justify-center gap-2">
            {ctaLabel}
